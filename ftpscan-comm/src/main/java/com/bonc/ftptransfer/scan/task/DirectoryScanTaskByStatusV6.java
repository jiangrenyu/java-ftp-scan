...
public class DirectoryScanTaskByStatusV6 implements Runnable {
	
	private static Logger LOG = LoggerFactory.getLogger(DirectoryScanTaskByStatusV6.class);
	...
	public DirectoryScanTaskByStatusV6(boolean isSFTP, int loginTimeout,int queryBatchSize, List<FtpPath> ftpPathList, JdbcUtils jdbcUtil,
		...
		if(isLogin){
			...
			for(FtpPath ftpPath : this.ftpPathList){
				try {
					...
					if(fileMap.get(ftpPath.getRemotePath()) == null) {
						fileMap.put(ftpPath.getRemotePath(), ftpServer.listFileAndDirectory(ftpPath.getRemotePath(),ftpPath.getFileRegular()));
					}else {
						List<HashMap<String, Object>> resultList = ftpServer.listFileAndDirectory(ftpPath.getRemotePath(), ftpPath.getFileRegular());
						if (resultList != null && resultList.size() > 0) {
							fileMap.get(ftpPath.getRemotePath()).addAll(ftpServer.listFileAndDirectory(ftpPath.getRemotePath(), ftpPath.getFileRegular()));
						}
					}
			...
			if(fileMap.size() > 0){
				...
				for(FtpPath ftpPath : this.ftpPathList){
					...
					List<HashMap<String, Object>> scanedList = fileMap.get(ftpPath.getRemotePath());
...
//								LogFile logFile = null;
								...
								if(logFileStatus == null){
									...
									if(!fileId.equals(oldFileId)){
										...
										//如果原来的状态不是未下载或下载中，则全部改成未下载，否则不改变
										if(!FtpFileStatus.UNDOWNLOAD.equals(oldFileStatus) && !FtpFileStatus.DOWNLOADING.equals(oldFileStatus)){
										...
										//fid相同，判断状态
										if(FtpFileStatus.FILENOTFUND.equals(logFileStatus.getStatus()) || FtpFileStatus.UPDATENOTDOWNLOADMANUAL.equals(logFileStatus.getStatus())){
	...
	public LogFile createLogFile(String fId,String fKey,HashMap<String, Object> scanedFile,FtpPath ftpPath){
		...
		newLogFile.setRemote_path((String)scanedFile.get("filePath")+(String)scanedFile.get("fileName"));//远端路径，有可能该文件位于扫描的某个目录下
		newLogFile.setLocal_path(ftpPath.getLocalPath()+(String)scanedFile.get("fileName"));
