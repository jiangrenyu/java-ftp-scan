...
public class DirectoryScanTaskByStatus implements Runnable {
	
	private static Logger LOG = LoggerFactory.getLogger(DirectoryScanTaskByStatus.class);
	...
	public DirectoryScanTaskByStatus(boolean isSFTP, int loginTimeout,int queryBatchSize, List<FtpPath> ftpPathList, JdbcUtils jdbcUtil,
...
//								LogFile logFile = null;
								...
								if(logFileStatus == null){
									...
									if(!fileId.equals(oldFileId)){
										...
										//fid相同，判断状态
										if(FtpFileStatus.DOWNLOADFAIL.equals(logFileStatus.getStatus()) || FtpFileStatus.FILENOTFUND.equals(logFileStatus.getStatus())){
											//状态为下载失败、文件不存在，则发下载消息，只将log_ftp_status中置为无效并插入新的记录，因为log_ftp_file中将fid作为主键
										...
										}else if(FtpFileStatus.MOVEDIRERROR.equals(logFileStatus.getStatus())){
											//状态为下载 成功Move失败,则发送move消息，只将log_ftp_status中置为无效并插入新的记录
											...
											String msg = generateMessage(newLogFile,ftpPath,KafkaMessageType.MoveMessage);
											String topic = ftpPath.getTopic();
											try {
												//发送消息
												producer.send(topic,msg);
												...
												if(logFileStatusService.updateLogFileStatus(logFileStatus)){
													//更新并保存成功
													LOG.info("将log_ftp_status 中原状态为 {} 的记录置为无效并插入新的有效记录成功，文件名为： {}，F_ID为 {}",oldFileStatus,fileName,fileId);
												}else{
													//更新并保存失败
													LOG.info("将log_ftp_status 中原状态为 {} 的记录置为无效并插入新的有效记录失败，文件名为： {}，F_ID为 {}",oldFileStatus,fileName,fileId);
												}
